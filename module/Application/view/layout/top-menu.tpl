    <div class="navbar-header">
       ...
       <div class="collapse navbar-collapse">
           <ul class="nav navbar-nav">
               {if !$auth->hasIdentity()}
                    <li>
                        <a href="{$this->url('home')}" data-toggle="tooltip" title="{t}Home{/t}"><span class="glyphicon glyphicon-home"></span></a>
                    </li>
               {else}
                    <li {if $module_name == 'Config' }class="active"{/if}>
                        <a href="{$this->url( 'config_ivrs', [ 'action'=> 'list' ])}">{t}Configurations{/t}</a>
                    </li>
                    {if $idty->getUpUserprofile()->getUpid() == 1}
                        <li {if $module_name == 'Admin' }class="active"{/if}>
                            <a href="{$this->url( 'admin_users', [ 'action'=> 'list' ])}">{t}Administrator{/t}</a>
                        </li>
                    {/if}
               {/if}
           </ul>
            ...
            {if $auth->hasIdentity()}
               <ul class="nav navbar-nav navbar-right">
                   <li {if $this->ngnview()->getController() == 'Profile\Controller\Profile' }class="active"{/if}>
                       <a href="{$this->url( 'profile', [ 'action'=>'index' ] )}" data-toggle="tooltip" title="{t}Profile{/t}"><span class="glyphicon glyphicon-user"></span></a>
                   </li>
                   <li>
                       <a href="{$this->url( 'auth', [ 'action' => 'logout'] )}" data-toggle="tooltip" title="{t}Log Out{/t}"><span class="glyphicon glyphicon-off"></span></a>
                   </li>
               </ul>
            {/if}
